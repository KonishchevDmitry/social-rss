...
def _photo(user, info, big, post_id=None):
    ...
    if post_id is None:
        text = _vk_link("photo", _vk_id(info["owner_id"], info["pid"]), _image(photo_src))
    else:
        # Photos from posts
        ...
        if info["pid"] == 0 or info["owner_id"] == 0:
            text = _vk_link("wall", _vk_id(user["id"], post_id), _image(photo_src))
        else:
            text = _vk_link(
                "wall", _vk_id(user["id"], post_id) +
                    "?z=photo" + _vk_id(info["owner_id"], info["pid"]) + "%2Fwall" + _vk_id(user["id"], post_id),
                _image(photo_src))
...
def _photo_item(users, user, item, title):
    ...
    return {
        ...
        "text":  "".join(_photo(user, photo, big_photos) for photo in photos),
    }
